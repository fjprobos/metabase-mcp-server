...
if (!process.env.JWT_SECRET) {
  console.error('WARN: JWT_SECRET is not set — using a random secret that will change on restart. Set JWT_SECRET in production.');
}
...
app.post('/oauth/register', (req: Request, res: Response) => {
  ...
  console.error(`INFO: Client registered: ${client_name || 'unnamed'} (${client_id})`);
  res.status(201).json({
    client_id,
...
app.listen(GATEWAY_PORT, () => {
  console.error(`INFO: OAuth MCP gateway on port ${GATEWAY_PORT}`);
  console.error(`INFO: Proxying /mcp  →  ${MCP_UPSTREAM}/mcp`);
  console.error(`INFO: Public URL: ${GATEWAY_URL}`);
});
